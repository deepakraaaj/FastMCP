...
from tag_fastmcp.models.contracts import AgentSelection, PolicyEnvelope, RequestContext, RoutingPayload
from tag_fastmcp.models.http_api import AdminChatResult
...
class AdminOrchestrationAgent:
    ...
    def _clarifier(self) -> ClarificationAgent:
    ...
    def _message_from_routings(self, routings: list[RoutingPayload]) -> str:
        ...
        for index, routing in enumerate(routings, start=1):
            ...
            if "report" in output:
                ...
                base = f"I ran report '{report['report_name']}' and found {report['row_count']} rows."
    ...
    async def _answer_only_reply(
        ...
        *,
        ...
        resolved_app_id = primary_app_id
        if resolved_app_id is None and len(allowed_app_ids) == 1:
            resolved_app_id = allowed_app_ids[0]

        if resolved_app_id is not None:
            current_snapshot = await self.session_store.get(session_id)
            history = self._history_for_agent(current_snapshot.history)
            app_ctx = self.app_router.resolve(resolved_app_id)
            return await self._clarifier().chat(app_ctx, message, history=history or None)

        app_scope = ", ".join(allowed_app_ids) if allowed_app_ids else "the current admin scope"
        return f"{fallback_reason} Current admin scope: {app_scope}."
    ...
    async def run(
        ...
        *,
        ...
        if started_session and context_message:
        ...
        render_payload: dict[str, Any] = {
            ...
            "primary_app_id": policy_envelope.primary_app_id,
        ...
        if compiled.orchestration_decision.orchestration_mode == "proposal":
            ...
            reply = await self._answer_only_reply(
                session_id=session_id,
                message=message,
                primary_app_id=policy_envelope.primary_app_id,
                allowed_app_ids=list(policy_envelope.allowed_app_ids),
                fallback_reason=compiled.orchestration_decision.user_visible_reason,
            )
        else:
        ...
        await self.session_store.append_event(
            session_id,
            {
                ...
                "content": message,
                "app_id": policy_envelope.primary_app_id,
                "scope_app_ids": list(policy_envelope.allowed_app_ids),
            },
        )
        await self.session_store.append_event(
            session_id,
        ...
        return AdminChatResult(
            ...
            metadata={
                ...
                "agent_runtime_state": agent_selection.runtime_state,
